...
let piBleManager = null;
let piBleLastPayload = null;
...
function isLocalMode() {
  return Boolean(window.APP_CONFIG?.allowLocalMode) || !Boolean(window.APP_CONFIG?.apiBaseUrl);
}
...
function updatePiConnectionUI(connected = null, message = null) {
  const status = document.getElementById("piConnectionStatus");
  const connectBtn = document.getElementById("connectPiBtn");
  const disconnectBtn = document.getElementById("disconnectPiBtn");

  const isConnected = connected === null
    ? Boolean(piBleManager && piBleManager.isConnected && piBleManager.isConnected())
    : connected;

  if (status) {
    status.textContent = message || (isConnected ? "Verbonden" : "Niet verbonden");
    status.classList.toggle("ok", isConnected);
    status.classList.toggle("bad", !isConnected);
  }
...
function ensurePiBleManager() {
  if (piBleManager) {
    return piBleManager;
  }

  if (!window.BLEManager) {
    addLogEntry("BLE manager niet geladen", "warning");
    return null;
  }

  piBleManager = new window.BLEManager();
  piBleManager.onConnectionChange = (connected) => {
    updatePiConnectionUI(connected);
    addLogEntry(connected ? "Pi BLE verbonden" : "Pi BLE verbroken", connected ? "success" : "warning");
  };
  piBleManager.onDetectionsUpdate = (payload) => {
    piBleLastPayload = payload;
    const latest = Array.isArray(payload?.detections) ? payload.detections[0] : null;
    if (!latest) {
      return;
    }

    const serviceLabel = latest.service || latest.emoji || "Pi";
    const distanceText = latest.distance_km !== undefined ? `${latest.distance_km} km` : "onbekende afstand";
...
async function connectPiBle() {
  ...
  updatePiConnectionUI(false, "Verbinden…");
  const connected = await manager.connect();
  if (connected) {
    const status = await manager.readStatus();
    if (status?.status) {
      updatePiConnectionUI(true, `Verbonden: ${status.status}`);
    } else {
      updatePiConnectionUI(true);
    }

    const payload = await manager.readDetections();
    if (payload) {
      piBleLastPayload = payload;
    }
  } else {
...
function disconnectPiBle() {
  if (!piBleManager) {
    updatePiConnectionUI(false);
    return;
  }

  piBleManager.disconnect();
  updatePiConnectionUI(false, "Niet verbonden");
}
...
on("connectPiBtn", "click", async () => {
  await connectPiBle();
});

on("disconnectPiBtn", "click", () => {
  disconnectPiBle();
});
